...
population_variance <-function(x) {
  # Thêm phần code của bạn vào đây
...
population_stdvar <-function(x) {
  # Thêm phần code của bạn vào đây
  return(sqrt(population_variance(x)))
}
...
```{r, echo=TRUE}
# Thêm phần code vẽ biểu đồ boxplot, histogram của age và fat vào đây
par(mfrow=c(2,2))
boxplot(age,fat)
hist(age)
hist(fat)
...
```{r, echo=TRUE}
# Thêm phần code vẽ biểu đồ boxplot, histogram của age và fat vào đây
plot(age,fat, xlab = "age", ylab = "fat",pch= 19)
abline(lm(fat~age))
```
...
```{r, echo=TRUE}
euclid <- function(x, y) {
  # Thêm phần định nghĩa hàm của bạn vào đây
...
$$minkowski(x,y) = \sqrt[p]{\sum_{i=1}^{n} (x_i - y_i)^p}$$
```{r, echo=TRUE}
...
minmax.norm <- function(x, newmin, newmax) {
  # Thêm phần code định nghĩa hàm của bạn ở đây
...
zscore.mad.norm <- function(x) {
  # Thêm phần code định nghĩa hàm của bạn ở đây
  mad=(1/length(x))*(sum(abs(x-mean(x))))
  return((x-mean(x))/mad)
}
...
$$x_i' = \frac{x_i - mean(x)}{stdvar(x)}$$
```{r, echo = TRUE}
zscore.sd.norm <- function(x) {
  # Thêm phần code định nghĩa hàm của bạn ở đây
  return((x-mean(x))/population_stdvar(x))
}
